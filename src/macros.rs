...
macro_rules! wren_load_module_fn {
    ($f:path) => {
        $crate::macros::_wrap_load_module_fn($f, $crate::macros::_default_realloc)
    };
    ($f:path, $alloc:path) => {
        $crate::macros::_wrap_load_module_fn($f, $alloc)
    };
}
...
#[doc(hidden)]
#[inline]
pub fn _wrap_load_module_fn<
    F: Fn(&mut VM, &str) -> Option<String>,
